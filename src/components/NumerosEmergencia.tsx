...
function NumberCard({ href, label, number, variant = 'default' }: NumberCardProps) {
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.97 }}
      className={`group flex flex-col p-3.5 rounded-sm border transition-colors ${
        isEmergency
    ...
    >
      ...
      <span className={`text-lg font-serif font-bold mt-1 ${
        isEmergency ? 'text-crisis-red-dark dark:text-crisis-red-light' : 'text-ink dark:text-ink-dark group-hover:text-crisis-red dark:group-hover:text-crisis-red-light transition-colors'
      }`}>
        {number}
      </span>
    </motion.a>
...
function NumberRow({ href, label, number }: NumberRowProps) {
  return (
    <motion.a
      ...
      className="group flex justify-between items-center bg-panel dark:bg-panel-dark border border-rule dark:border-rule-dark p-3.5 rounded-sm hover:border-crisis-red/20 dark:hover:border-crisis-red/30 hover:bg-paper dark:hover:bg-paper-dark transition-all duration-200"
    >
      ...
      <div className="flex gap-2">
        {numbers.map((item) => (
          <motion.a
            ...
            whileHover={{ y: -1.5 }}
            whileTap={{ scale: 0.98 }}
            className="flex-1 text-center py-2 bg-paper dark:bg-paper-dark border border-rule dark:border-rule-dark rounded-sm text-small font-serif font-bold text-ink dark:text-ink-dark hover:border-crisis-red/20 dark:hover:border-crisis-red/30 hover:bg-panel dark:hover:bg-panel-dark transition-all duration-200"
          >
            {item.number}
          </motion.a>
        ))}
...
function PlatformCard({ href, icon, title, description, badgeColor = 'blue' }: { href: string; icon: React.ReactNode; title: string; description: string; badgeColor?: 'blue' | 'purple' }) {
  ...
  return (
    <motion.a
      ...
      whileHover={{ y: -2 }}
      whileTap={{ scale: 0.98 }}
      className="group flex items-center justify-between bg-panel dark:bg-panel-dark border border-rule dark:border-rule-dark p-3.5 rounded-sm hover:border-crisis-red/20 dark:hover:border-crisis-red/30 hover:bg-paper dark:hover:bg-paper-dark transition-all duration-200"
    >
        ...
        {isOpen && (
          <div className="fixed inset-0 z-[1100] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <motion.div
              ...
              className="relative bg-panel dark:bg-panel-dark w-full sm:max-w-lg h-[85vh] sm:h-auto sm:max-h-[85vh] rounded-t-lg sm:rounded-sm shadow-2xl flex flex-col border-t-2 border-crisis-red overflow-hidden z-10"
            >
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="bg-panel dark:bg-panel-dark border border-rule dark:border-rule-dark p-3.5 rounded-sm flex flex-col gap-2.5">
                      ...
                      <motion.a
                        ...
                        whileHover={{ y: -1.5 }}
                        whileTap={{ scale: 0.98 }}
                        className="block text-center py-2 bg-paper dark:bg-paper-dark border border-rule dark:border-rule-dark rounded-sm text-small font-serif font-bold text-ink dark:text-ink-dark hover:border-crisis-red/20 dark:hover:border-crisis-red/30 hover:bg-panel dark:hover:bg-panel-dark transition-all duration-200"
                      >
